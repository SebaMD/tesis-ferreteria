...
export default function ClientCartPage() {
  ...
  useEffect(() => {
    getCatalogProductsRequest()
      ...
      .catch((error) => toast.error(getApiError(error, "No se pudo actualizar la disponibilidad")))
  ...
  const rows = items.map((item) => {
    ...
    const available = Boolean(liveProduct && Number(liveProduct.currentStock) > 0);
    const quantity = Math.min(Number(item.quantity), Number(product.currentStock || 0));
    return { ...item, product, available, requestedQuantity: Number(item.quantity), quantity };
  });
  ...
  const changeQuantity = (row, quantity) => {
    ...
    if (!updateQuantity(row.product.id, quantity, row.product.currentStock)) {
      toast.error(`Solo hay ${row.product.currentStock} unidades disponibles`);
    }
  };
    ...
    <main className="mx-auto grid w-full max-w-280 gap-5 px-6 py-8 max-[720px]:px-3.5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1.5 mb-0 text-sm text-slate-500">Selección temporal de productos, sin reserva ni pedido.</p>
        </div>
        {items.length > 0 && (
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_300px] items-start gap-5 max-[860px]:grid-cols-1">
          <section className="grid gap-3">
            {rows.map((row) => {
              ...
              const invalidQuantity = row.requestedQuantity > Number(row.product.currentStock);
                ...
                <article className="grid grid-cols-[96px_minmax(0,1fr)_auto] items-center gap-4 rounded-lg border border-slate-200 bg-white p-4 max-[620px]:grid-cols-[76px_1fr]" key={row.product.id}>
                  ...
                  <div className="grid gap-1.5">
                    ...
                    <span className={`text-xs font-bold ${row.available ? "text-positive-600" : "text-critical-600"}`}>
                      {row.available ? `${row.product.currentStock} disponibles` : "Producto no disponible actualmente"}
                    </span>
                    {invalidQuantity && <span className="text-xs text-critical-600">La cantidad fue limitada al stock actual.</span>}
                  </div>
                  <div className="grid justify-items-end gap-3 max-[620px]:col-span-2 max-[620px]:w-full max-[620px]:grid-cols-[1fr_auto] max-[620px]:items-center">
                    <div className="flex items-center gap-1">
                      <button className="size-9 min-h-9 border-slate-300 bg-white p-0 text-ink-700" type="button" onClick={() => changeQuantity(row, row.quantity - 1)} disabled={!row.available} aria-label="Disminuir cantidad"><Minus size={16} /></button>
                      <input className="w-16 text-center" type="number" min="1" max={row.product.currentStock} value={row.quantity} onChange={(event) => changeQuantity(row, Number(event.target.value))} disabled={!row.available} aria-label={`Cantidad de ${row.product.name}`} />
                      <button className="size-9 min-h-9 border-slate-300 bg-white p-0 text-ink-700" type="button" onClick={() => changeQuantity(row, row.quantity + 1)} disabled={!row.available || row.quantity >= Number(row.product.currentStock)} aria-label="Aumentar cantidad"><Plus size={16} /></button>
                    </div>
          ...
          <aside className="sticky top-22 grid gap-4 rounded-lg border border-slate-200 bg-white p-5 max-[860px]:static">
            ...
            <p className="m-0 rounded-[5px] bg-rust-50 px-3 py-3 text-xs leading-5 text-rust-700">
              El checkout y los pedidos online todavía no están habilitados. Tu selección quedará guardada en este navegador.
            </p>
